    ...
    <div className="flex min-h-screen items-center justify-center p-4" style={{ background: 'var(--bg)' }}>
      <div className="w-full max-w-[400px] animate-fadeUp">
        ...
        <div className="rounded-vault p-8" style={{ background: 'var(--sur)', border: '1px solid var(--bdr2)' }}>
          <h2 className="font-display text-xl font-light mb-6" style={{ color: 'var(--cream)' }}>Welcome back</h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            >
              {loading ? '...' : magicLinkMode ? 'Send Magic Link' : 'Sign In'}
            </button>
          </form>

          <button
            onClick={() => { setMagicLinkMode(!magicLinkMode); setError(''); setMessage(''); }}
            className="w-full mt-3 text-center font-body text-xs transition-colors hover:underline"
        ...
        <p className="text-center mt-6 font-body text-sm" style={{ color: 'var(--txt2)' }}>
          ...
          <Link to="/register" className="transition-colors hover:underline" style={{ color: 'var(--gold)' }}>Sign up</Link>
        </p>
      </div>
    </div>
  );
